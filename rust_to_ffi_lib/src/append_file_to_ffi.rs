...
async fn main_(input_operator: &str, input_address: &str, input_port: &str, input_operator_secret: &str, input_path_to_file: &str) {) -> Result<(), Error> {
    ...
    // append to a file
    let file = input_path_to_file.parse()?;
    ...
    let id = await!(client
        ...
        .sign(&input_operator_secret?.parse()?) // sign as the owner of the file to approve the change
        .execute_async())?;
...
fn output_append_file(input_operator: &str, input_address: &str, input_port: &str, input_operator_secret: &str, input_path_to_file: &str) {
